-- Advancers v2: rewire submit_predictions and admin_submit_predictions
-- to accept `advancers: [{rank, team_id}, ...]` instead of the old
-- `bundles: [{slot_index, group_letter}]`. Add admin_set_tournament_advancer
-- and admin_clear_tournament_advancer (replacing admin_set_third_place_advancer
-- which was dropped in 0025).
--
-- Candidate-pool rule for users: each advancer team_id must appear in the
-- prediction's own group_predictions as a 3rd-place pick. For admins
-- (admin_set_tournament_advancer): team_id must appear in group_standings
-- as a 3rd-place team for the tournament.
...
-- ========== admin_set_tournament_advancer ==========
-- Candidate pool: teams listed as 3rd-place in group_standings.

create or replace function public.admin_set_tournament_advancer(
...
begin
    ...
    if p_rank < 1 or p_rank > 8 then
        raise exception 'advancer rank must be 1..8 (got %)', p_rank using errcode = 'P0001';
    end if;

    select exists(
        select 1 from public.group_standings
         where tournament_id = p_tournament_id
           and third_team_id = p_team_id
    ) into v_is_third;
    if not v_is_third then
        raise exception 'team % is not a 3rd-place team in this tournament', p_team_id
            using errcode = 'P0001';
    end if;

    begin
        insert into public.tournament_advancers (tournament_id, rank, team_id)
        values (p_tournament_id, p_rank, p_team_id)
        on conflict (tournament_id, rank)
        do update set team_id = excluded.team_id;
    exception
        when unique_violation then
            raise exception 'team % is already assigned to a different rank', p_team_id
                using errcode = 'P0001';
    end;
end;
$$;

grant execute on function public.admin_set_tournament_advancer(uuid, int, text) to authenticated;

-- ========== admin_clear_tournament_advancer ==========
-- Also drops any r32_bracket_assignments that referenced the cleared team
-- so the bracket stays consistent.

create or replace function public.admin_clear_tournament_advancer(
    p_tournament_id uuid,
    p_rank int
)
returns void
...
begin
    ...
    select team_id into v_team_id
     ...
     where tournament_id = p_tournament_id and rank = p_rank;

    if v_team_id is not null then
        delete from public.r32_bracket_assignments
         where tournament_id = p_tournament_id and team_id = v_team_id;
    end if;

    delete from public.tournament_advancers
     where tournament_id = p_tournament_id and rank = p_rank;
end;
$$;

grant execute on function public.admin_clear_tournament_advancer(uuid, int) to authenticated;
