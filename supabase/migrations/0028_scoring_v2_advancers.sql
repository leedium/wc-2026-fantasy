-- Advancers v2: rewire get_leaderboard / get_leaderboard_rank to score
-- the new ranked top-8 advancer picks.
--
-- Scoring rule (per advancer pick):
--   * predicted team in the actual top-8 AND at the same rank → 0.5
--   * predicted team in the actual top-8 (any rank)            → 0.25
--   * else                                                     → 0
-- Max per pick = 0.5; max total advancer points = 4. This preserves the
-- ~4-point advancer ceiling from the old bundle scoring (which was
-- 0.5 × 8 = 4).
--
-- The output column is renamed `bundle_points` → `advancer_points` to
-- reflect the new model. Callers (frontend leaderboard route, types,
-- LeaderboardTable component) are updated in the same PR.
...
as $$
    ...
    advancer_scores as (
        select
            ...
            coalesce(sum(
                case
                    when ta_rank.team_id is not null and ta_rank.team_id = ap.team_id then 0.5
                    when ta_team.team_id is not null then 0.25
                    else 0
                end
